...
<body>
        ...
        <div class="atas" style="margin: 0px 1.5em;">
            <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
                ...
                <div class="carousel-inner container">
                    <?php
                    $ambildata = mysqli_query($conn, "SELECT * FROM content WHERE id='41'");
                    $i = "false";
                    while ($tampil = mysqli_fetch_array($ambildata)) {
                        ...
                        $i = "true";
                        echo "
                        <img class='d-block w-100' src='../image/$tampil[image]' width='800' height='250' alt='$tampil[content]'>
                    </div>
                    ";
                    }
                    ?>
    ...
    <div class="container-costum">
        <div class="justify-content-center pl-3 pr-3 pt-5 pb-5">
            <h4 style="text-align: center;">LAMBORGHINI</h4>
            <h4 style="text-align: center;">UPTD Puskesmas Cilacap Utara I</h4>
            <?php
            include "../config/koneksi.php";
            $ambildata = mysqli_query($conn, "SELECT * FROM content WHERE id='42'");
            while ($tampil = mysqli_fetch_array($ambildata)) {
                echo "
                <div class='card' style='text-align: center;>   
                    <h5 class='card-title'>$tampil[title]</h5>
                    <p style='text-align: justify;'>$tampil[content]</p>
                <img class='card-img-top' src='../image/$tampil[image]' width = '400px' height='500px' alt=''>
                </div> ";
            ?>
            <?php 
            } ?>
            ...
            <?php
            include "../config/koneksi.php";
            $ambildata = mysqli_query($conn, "SELECT * FROM content WHERE id='83'");
            while ($tampil = mysqli_fetch_array($ambildata)) {
                echo "
                <div class='card' style='text-align: center;>   
                    <h5 class='card-title'>$tampil[title]</h5>
                    <p style='text-align: justify;'>$tampil[content]</p>
                <img class='card-img-top' src='../image/$tampil[image]' width = '400px' height='950px' alt=''>
                </div> ";
            ?>
            <?php 
            } ?>
        ...
        <div class="content row workingspace">
            <div class="col-6">
            <h4 style="text-align: center;">Produsen Tahu dan Tempe</h4>
                <?php
                $ambildata = mysqli_query($conn, "SELECT * FROM content WHERE id='81'");
                while ($tampil = mysqli_fetch_array($ambildata)) {
                    echo "
            ...
            <h4 style="text-align: center;">Produsen Tahu dan Tempe</h4>
                <?php
                $ambildata = mysqli_query($conn, "SELECT * FROM content WHERE id='82'");
                while ($tampil = mysqli_fetch_array($ambildata)) {
                    echo "
                    <img src='../image/$tampil[image]' alt='workingspace' class='img-fluid'>
                     ";
                ...
                ?>
                <?php if ("true" === $admin) {
                    $idedit = 'isilamborghini';
                    echo
                    "<button id='btn2'><a href='edit_lamborghini.php?idlamborghini=$idlamborghini'>Edit</a></button>";
                }
                ?>
            </div>
        </div>        
        ...
        <?php
            $ambildata = mysqli_query($conn, "SELECT * FROM video WHERE id='17'");
            while ($tampil = mysqli_fetch_array($ambildata)) {
