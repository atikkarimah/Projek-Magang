...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
        $sql = "UPDATE content SET content = '" . $_POST["motto"] . "', image='$newfilename' WHERE type='isimotto'";
    } else {
        $sql = "UPDATE content SET content = '" . $_POST["motto"] . "', image= '" . $_POST["tempimage"] . "' WHERE type='isimotto'";
    }

    if ($conn->query($sql) === TRUE && $conn->query($sqlmotto) === TRUE) {
        echo "<script>alert('Success!');window.location.href='visimisi.php';
        </script>";
    } else echo "gagal";
    $conn->close();
...
<body class="container">
    <div id="content">
        ...
        $ambilmotto = mysqli_query($conn, "SELECT * FROM content WHERE type='isimotto'");
        ?>
        <form method="POST" enctype="multipart/form-data">
            ...
            <?php
            if ($tampil = mysqli_fetch_array($ambilmotto)) { ?>
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label">MOTTO</label>
                    <div class="col-sm-10">
                        <textarea type="textarea" class="form-control" name="motto" style="width:500px; height:300px;"><?php echo $tampil['content'] ?></textarea>
                    </div>
                </div>
            <?php } ?>
            <button type="submit" class="btn btn-primary mb-2" name="submit">Submit</button>
