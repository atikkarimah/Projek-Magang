...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
        $sql = "UPDATE sugesti SET title='$title', content='$content', image='$newfilename' WHERE id='$idsugesti'";
    } else {
        $sql = "UPDATE sugesti SET title='$title', content='$content',  image= '" . $_POST["tempimage"] . "' WHERE id='$idsugesti'";
    }



    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Success!');window.location.href='sugesti.php';
        </script>";
    } else echo "gagal";
    $conn->close();
}
        ...
        ?>
            <form method="POST" enctype="multipart/form-data">
                <div class="justify-content-center pl-3 pr-3 pt-5 pb-5">
                    <h4 style="text-align: center;">Edit Sugesti</h4>
                </div>
